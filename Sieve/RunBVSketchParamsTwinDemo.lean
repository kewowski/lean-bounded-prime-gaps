/-
  Sieve/RunBVSketchParamsTwinDemo.lean
  Demo: with BVParams and an assumed analytic inequality, build
  `AI := bvSketchParams` and derive ≥1 / ≥2 consequences in the twin window.
-/
import Mathlib
...
example
  ...
  (proof :
    ∀ (W : Sieve.MaynardWeights.Weight) (τ : ℝ)
      ...
      Sieve.Analytic.lowerFormulaParams P W τ H HS hne
  ...
  (h1 :
    (1 : ℝ) ≤
      (Sieve.Analytic.bvSketchParams P TB proof).lower W τ
  ...
  classical
  let AI := Sieve.Analytic.bvSketchParams P TB proof
  simpa using
